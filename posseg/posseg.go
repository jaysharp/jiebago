...
func (seg *Segmenter) LoadMyDictionary(fileName string) error {
	seg.Dict = &MyDictionary{
		Dictionary: *CommonDictIns(fileName),
		newFreqMap: make(map[string]float64),
		newPosMap:  make(map[string]string),
	}
...
func (seg *Segmenter) LoadDictionary(fileName string) error {
	seg.Dict = &MyDictionary{
		Dictionary: Dictionary{FreqMap: make(map[string]float64), PosMap: make(map[string]string)},
	}
